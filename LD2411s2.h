...
class UARTSensor2 : public Component, public UARTDevice {
  public:
  UARTSensor2(UARTComponent *parent) : UARTDevice(parent) {}
  Sensor* presence_sensor2= new Sensor();
  Sensor* motion_sensor2 = new Sensor();
  Sensor* distance_sensor2 = new Sensor();
  Sensor* max_motion_sensor2 = new Sensor();
  Sensor* min_motion_sensor2 = new Sensor();
  Sensor* max_presence_sensor2 = new Sensor();
  Sensor* min_presence_sensor2 = new Sensor();
  Sensor* unocc_time_sensor2 = new Sensor();
...
void loop() override
{
  ...
  void processPacket() {            
	...
	if ((bytes[0] == 0xAA) && (bytes[1] == 0xAA) && (bytes[2] == 0x00)) { 
		...
		presence_sensor2->publish_state(0);
		motion_sensor2->publish_state(0);
  ...
  if ( (bytes[0] == 0xAA) && (bytes[1] == 0xAA) && (bytes[2] == 0x01) ) {        
		...
		presence_sensor2->publish_state(1);
		motion_sensor2->publish_state(1);
    ...
    unsigned char byte4 = bytes[4];
		...
		distance_sensor2->publish_state(distanceCm);
		return;
	}
  ...
  if ( (bytes[0] == 0xFD) && (bytes[1] == 0xFC) && (bytes[2] == 0xFB) && (bytes[3] == 0xFA) && (bytes[6] == 0x73) && (bytes[7] == 0x01) ) {
    ...
    int unocc_time2 = static_cast<int>(unocchex);

    max_motion_sensor2->publish_state(max_m);
		min_motion_sensor2->publish_state(min_m);
		max_presence_sensor2->publish_state(max_p);
		min_presence_sensor2->publish_state(min_p);
		unocc_time_sensor2->publish_state(unocc_time2 / 10);
